...
class EvolutionContractTests(unittest.TestCase):
    ...
    def test_fiat_state_shape_frontier_holds_the_task_identity_successor(self):
        ...
        self.assertEqual(field(ledger, "Current version"), "fiat-v5.27.1")
        self.assertEqual(field(ledger, "Frontier status"), "open")
        ...
        latest = history_rows(ledger)[-1]
        self.assertEqual(latest["version"], "fiat-v5.27.1")
        self.assertEqual(latest["axis"], "generation")
        ...
        self.assertIn("skills#617", latest["evidence"])
        self.assertIn("fiat-host-byline-readback", latest["evidence"])
        predecessor = history_rows(ledger)[-2]
        self.assertEqual(predecessor["version"], "fiat-v5.26.1")
        self.assertIn("skills#429", predecessor["evidence"])
        self.assertIn("pull/552", predecessor["evidence"])
        self.assertIn("audit-record-schema-timestamp-synopsis-recovery", predecessor["evidence"])
        earlier = history_rows(ledger)[-3]
        self.assertEqual(earlier["version"], "fiat-v5.25.1")
        self.assertIn("skills#495", earlier["evidence"])
        self.assertIn("fiat-run-worktree-study.md", earlier["evidence"])
