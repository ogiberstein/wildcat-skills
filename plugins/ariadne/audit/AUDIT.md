...
This file is the record of the build that produced this plugin. Runs that came
afterwards record their rounds in the marketplace repository's own
`audit/AUDIT.md`, under a heading naming the run, and the dataset and
state-fixture predicate runs are both there. That file is not linked from here,
because this directory is published on its own and a link leaving it would not
resolve. Six sections of the state-fixture run were written here by mistake and
moved; this paragraph exists so the next run does not repeat it.
