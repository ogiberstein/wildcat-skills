...
- Promise: A completed X-Ray run establishes that the required pre-audit views were produced for the named repository snapshot and complete current logical scope, with their source boundary and unresolved gaps visible; when source reuse is selected, it applies only to validated per-source preparation entries.
- Evidence: The digest-matched vendored instruction; the first-party adapter `plugins/hexaemeron/lib/xray_reuse.py` at SHA-256 `0472434add5cc9feba284e1a123d5e769d6da530fe7d6d834b4723e04760c809`; the repository commit, complete current scope and source digests, validated plan, exact current union, fresh global synthesis, and output manifest; and current-run `architecture.json`, `x-ray.md`, `entry-points.md`, and `invariants.md` with their cited inputs and digests.
...
- Consequence: 1
- Refuses: A stale X-Ray or adapter digest, an unnamed repository snapshot, an unsafe or incomplete current scope, a prior/current scope mismatch or other cache uncertainty that does not produce named full recomputation, a stale preparation entry, a union that differs from the exact current inventory, missing fresh global synthesis, a missing or unbound required artefact, uncited conclusions, hidden exclusions, or language that presents pre-audit analysis as security sign-off.
- Recovery: Reconcile both instruction digests, repair the source boundary, discard uncertain cache material and run the named full recomputation, then regenerate fresh global synthesis and all four views and leave every unresolved gap explicit.
- Exceptions: none
