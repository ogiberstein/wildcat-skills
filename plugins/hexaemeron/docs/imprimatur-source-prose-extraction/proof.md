...
This record preserves the Step 2 demonstration for [issue 503](https://github.com/wildcat-finance/skills/issues/503), alongside the accepted [study](study.md) and [runbook](runbook.md). It reports the named commands and fixture bytes observed on 2026-08-26; it does not extend those observations to other source, toolchains, or executions.

## Recorded identities

The demonstration started from signed Step 1 commit `10b4d7f04ca52abfe6aeafa0e8c2c0db5dcdf566` with Python `3.12.3` and Node `v26.6.0`. Node `v26.6.0` was placed first on `PATH` for the Hexaemeron suite.
...
| `plugins/hexaemeron/.claude-plugin/plugin.json` | package | `1.5.10` |
| `plugins/hexaemeron/.codex-plugin/plugin.json` | package | `1.5.10` |
| `.claude-plugin/marketplace.json` | package listing | `1.5.10` |
| `.agents/plugins/marketplace.json` | package listing | `1.5.10` |
| `tests/test_version_propagation.py` | package assertion | `1.5.10` |

The package moved by one patch from `1.5.9`. That distribution identity is separate from the Imprimatur generation and does not move its held frontier.
