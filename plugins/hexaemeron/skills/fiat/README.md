...
2. If `.hexaemeron/state.json` exists, run `hexctl verify`, then
   ...
   archive the completed run and continue immediately as a new run. If the
   phase is not `done`, resume it and treat the state file as canonical.
...
1. Confirm `git` and `gh auth status` work in the target directory. Resolve
   the repository from the current directory and the user's named target;
   never substitute a related-looking organisation or repository.
...
4. Nothing else. Fiat has no issue phase. Ignore ambient issue-first
   conventions for this workflow. Only an explicit higher-priority policy in
   the target repository can override this rule.
...
Branch as `step-<n>-<slug>` from the configured base.
...
URL. Pushing opens a PR; it never merges one.
...
hand over: topic, step list with PR URLs, audit rounds per step
