...
- A runtime host or model -- including Claude, Codex, ChatGPT, Copilot, or a
  similar successor -- is neither author nor co-author. Do not carry a
  host-generated byline into the commit or pull-request body.
- Only a run contributed by Shoggoth needs the Shoggoth publication identity.
  A cloud host that cannot use that signer and GitHub account stops before
  publication and hands the exact branch or patch to a Shoggoth environment.
  The handoff is a publication boundary, not permission to share the private
  key.
...
- `_Generated by [Claude Code](https://claude.ai/code/session_...)_`: the
  session link a cloud or Remote Control session appends.
