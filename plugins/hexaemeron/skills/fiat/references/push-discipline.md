...
- Attribute agent-produced run work to `Shoggoth
  <shoggoth@wildcat.finance>`. Preserve a human contributor as author when the
  work is theirs. A runtime host or model -- including Claude, Codex, ChatGPT,
  Copilot, or a similar successor -- is neither author nor co-author.
- Do not carry a host-generated byline into the commit or pull-request body.
  A cloud host that cannot use the Shoggoth signer and GitHub account stops
  before publication and hands the exact branch or patch to a Shoggoth
  environment. The handoff is a publication boundary, not permission to share
  the private key.
...
- Before accepting an implement commit, an audit-fix commit, or the final
  ...
  copy of each trailer above. It rejects a known runtime host as author or
