...
### Amendment -- 2026-08-21 (second)

**What changed.** A rule record carries the source's own words rather than the twenty-field shape the source proposes. Measured across the pinned document, each rule is a heading, a `priority · grade · automation` triple, and a body of 11 to 59 words with a median of 22; two of the 120 carry a list of any kind. So the record keeps `statement`, the body verbatim with its footnote markers lifted into `references`, and `obligations`, a list whose every entry has to be an exact substring of that statement. The fields `summary`, `mechanism`, `recommendation`, `detector_signals`, `preconditions`, `proof_obligations`, `failure_modes` and `validation` leave the schema.

**Why.** Item 1 said the corpus would carry each rule's preconditions and proof obligations. The source states those for exactly one rule, STO-09, as the worked example of its proposed schema; for the other 119 they would have been mine. Assumption 5 forbids that, and a corpus whose fields are half transcription and half invention cannot be told apart later by anyone reading it. The substring rule is what makes an obligation provably the rule's own requirement rather than a paraphrase, and it is checked rather than asserted: a test compares every `statement` against the body the source states for that id, and every obligation against its statement.
