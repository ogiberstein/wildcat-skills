...
Recorded repository paths use Unicode scalar values in NFC and exclude
control, format, and surrogate characters. Each segment is also bounded
to 255 UTF-8 bytes and the complete relative path to 4,096 UTF-8 bytes, so a
scalar-count-valid name cannot inflate past either byte boundary. Validation
never silently normalises a different spelling. A clean
result is bound to one bounded final reopen and reread of the caller-named
file: byte count, digest, confinement, and file identity must match the
validated snapshot. The observation ends at that final reread; it is not a
continuing lock against later writers. The repository-owned Elenchus reporter
applies the same bounded final-byte and identity check to its named report.
...
- **Silently normalise repository paths or trust file identity alone.** This
  would accept a spelling other than the one recorded and would miss
  equal-length same-inode rewrites. Refusal plus one bounded final reread keeps
  the checked bytes explicit without an unbounded stability loop.
...
and token facts remain source-bound exposed values or unknown; explicit
estimates refuse.
